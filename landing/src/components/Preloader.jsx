...
export default function Preloader({ onDone }) {
  ...
  useEffect(() => {
    if (!root.current || !topRef.current || !botRef.current || !wordRef.current) return
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to(topRef.current, { y: -28, rotate: -8, duration: 0.7, ease: 'power3.inOut' }, 0.4)
        .to(botRef.current, { y: 28, rotate: 6, duration: 0.7, ease: 'power3.inOut' }, 0.4)
        .to([topRef.current, botRef.current], { opacity: 0, duration: 0.4 }, 0.9)
        .from(wordRef.current, { opacity: 0, y: 20, duration: 0.6, ease: 'power3.out' }, 0.85)
  ...
  return (
    <div ref={root} style={{
      ...
      <div style={{ position: 'relative', width: 220, height: 160, display: 'grid', placeItems: 'center' }}>
        <svg ref={topRef} width="120" height="80" viewBox="0 0 120 80" style={{ position: 'absolute', top: 0 }}>
          <path d="M10 78 C 18 30, 42 4, 60 4 S 102 30, 110 78 L 100 70 L 88 78 L 76 70 L 64 78 L 52 70 L 40 78 L 28 70 L 18 78 Z"
            fill="#F5EFE6" />
        </svg>
        <svg ref={botRef} width="120" height="80" viewBox="0 0 120 80" style={{ position: 'absolute', bottom: 0 }}>
